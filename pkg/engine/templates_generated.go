...
- path: /opt/azure/containers/setup-etcd.sh
  ...
  {{- if IsMasterVirtualMachineScaleSets}}
    ...
    sudo sed -i "1iETCDCTL_CACERT={{WrapAsVariable "etcdCaFilepath"}}" /etc/environment
    sudo sed -i "1iETCDCTL_KEY={{WrapAsVariable "etcdClientKeyFilepath"}}" /etc/environment
    sudo sed -i "1iETCDCTL_CERT={{WrapAsVariable "etcdClientCertFilepath"}}" /etc/environment
